...
export default function ProductsIndex({ auth, products, categories, filters, totalStats }) {
    ...
    const [viewMode, setViewMode] = useState(() => {
        ...
        // Default to 'grid' to match the 16 items layout preference
    ...
    const ProductCard = ({ product }) => {
        // แสดง Lot ที่เก่าที่สุดก่อน (FIFO) - quantity > 0
        const activeLot = product.stock_lot_instances && product.stock_lot_instances.length > 0
            ? product.stock_lot_instances
                ...
                <div className="relative h-48 bg-gradient-to-br from-blue-100 to-purple-100">
                    ...
                    {activeLot && (
                        <div className="absolute bottom-3 right-3">
                            <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-purple-200 text-purple-900 whitespace-nowrap">
                                {activeLot.lot_number} • {formatNumber(activeLot.quantity)}
                            </span>
                        </div>
                    )}
                </div>
                ...
                <div className="p-6">
                    ...
                    <div className="flex justify-between items-center text-sm mb-3">
                        <span className="text-orange-600">
                            ต้นทุน: {product.cost_price && product.cost_price > 0 ? formatCurrency(product.cost_price) : '-'}
                        </span>
                        <span className="text-purple-600">
                            กำไร: {product.profit_margin ? `${product.profit_margin}%` : '-'}
                        </span>
                    </div>
                    ...
                    {/* Lot Information */}
                    {activeLot && (
                        <div className="bg-purple-100 rounded p-2 mb-3 border border-purple-300">
                            <div className="text-xs text-purple-900">
                                <span className="font-medium">{activeLot.lot_number}</span>
                                <span className="mx-1">•</span>
                                <span>{formatNumber(activeLot.quantity)} ชิ้น</span>
                                {activeLot.expiry_date && (
                                    <>
                                        <span className="mx-1">•</span>
                                        <span>{formatDate(activeLot.expiry_date)}</span>
                                    </>
                                )}
                            </div>
                        </div>
                                    ...
                                    <table className="min-w-full">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {products.data.map((product) => {
                                                ...
                                                const latestMovement = !activeLot && product.stock_movements && product.stock_movements.length > 0 
                                                ...
                                                return (
                                                    <tr key={product.id} className="hover:bg-gray-50 transition-colors duration-200">
                                                        ...
                                                        <td className="px-6 py-4">
                                                            <div className="text-sm font-medium text-purple-600">{product.profit_margin ? `${product.profit_margin}%` : '-'}</div>
                                                        </td>
                                                        <td className="px-6 py-4">
                                                        ...
                                                        <td className="px-6 py-4">
                                                            {activeLot || lotInfo ? (
                                                                <div className="text-xs text-gray-700">
                                                                    <span className="font-medium text-purple-900">{activeLot?.lot_number || lotInfo?.lot_number}</span>
                                                                    <span className="mx-1">•</span>
                                                                    <span>{activeLot ? formatNumber(activeLot.quantity) : 'N/A'} ชิ้น</span>
                                                                    {(activeLot?.expiry_date || lotInfo?.expiry_date) && (
                                                                        <>
                                                                            <span className="mx-1">•</span>
                                                                            <span className="text-gray-600">{formatDate(activeLot?.expiry_date || lotInfo?.expiry_date)}</span>
                                                                        </>
                                                                    )}
                                                                </div>
                                                            ) : (
